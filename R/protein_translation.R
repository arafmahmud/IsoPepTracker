...
check_bsgenome_availability <- function() {
  tryCatch({
    # Force library loading with verbose feedback
    cat("Loading BSgenome.Hsapiens.UCSC.hg38...\n")
    library(BSgenome.Hsapiens.UCSC.hg38, quietly = FALSE)
    cat("Loading rtracklayer...\n")
    library(rtracklayer, quietly = FALSE)
    cat("Loading Biostrings...\n")
    library(Biostrings, quietly = FALSE)
    cat("Loading GenomicRanges...\n")
    library(GenomicRanges, quietly = FALSE)
    cat("All BSgenome libraries loaded successfully!\n")
    return(TRUE)
  }, error = function(e) {
...
translate_single_isoform <- function(cds_gtf, isoform_type) {
  ...
  tryCatch({
    ...
    if (length(full_cds) == 0) {
      ...
      # Ensure we have a proper DNAString object for translation
      tryCatch({
        # Convert to proper DNAString if needed
        if (!inherits(full_cds, "DNAString")) {
          full_cds <- Biostrings::DNAString(as.character(full_cds))
        }
        protein <- as.character(Biostrings::translate(full_cds))
        protein_length <- nchar(protein)
      }, error = function(e) {
        cat("    TRANSLATION ERROR:", e$message, "\n")
        cat("    Sequence class:", class(full_cds), "\n")
        cat("    Sequence length:", length(full_cds), "\n")
        # Try alternative approach
        tryCatch({
          seq_char <- as.character(full_cds)
          full_cds_fixed <- Biostrings::DNAString(seq_char)
          protein <- as.character(Biostrings::translate(full_cds_fixed))
          protein_length <- nchar(protein)
          cat("    SUCCESS: Fixed sequence and translated\n")
        }, error = function(e2) {
          stop("Cannot translate sequence: ", e2$message)
        })
      })
...
build_correct_cds_sequence <- function(cds_ranges, phases, strand_info) {
  ...
  # Extract all sequences using BSgenome (handles strand automatically)
  sequence_set <- getSeq(BSgenome.Hsapiens.UCSC.hg38, cds_ranges)
  ...
  # Concatenate all processed sequences to form complete CDS
  if (length(processed_sequences) == 1) {
    complete_cds <- processed_sequences[[1]]
  } else {
    # Convert all sequences to character then back to DNAString for robust concatenation
    seq_chars <- sapply(processed_sequences, as.character)
    complete_cds <- Biostrings::DNAString(paste(seq_chars, collapse = ""))
  }
