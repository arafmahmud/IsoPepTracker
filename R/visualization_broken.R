...
get_transcript_peptides_genomic <- function(txID, processed_data, protease = "trp") {
  ...
  # cat("Found", length(mapped_ranges), "mapped peptides for", txID, "enzyme", protease, "\n")
  return(list(
    peptides = peptides,
...
create_peptide_plot_data <- function(exons_result, transcript_ids, gene_symbol, gene_id, processed_data, protease) {
  ...
  # DEBUG: Print the actual coordinates being used for visualization
  # cat("DEBUG: Visualization plot coordinates:\n")
  # cat("  Exon ranges:", paste(start(all_ranges), "-", end(all_ranges), collapse = ", "), "\n")
  # cat("  Gene start (with padding):", gene_start, "\n")
  # cat("  Gene end (with padding):", gene_end, "\n")
  ...
  for (i in seq_along(transcript_ids)) {
    ...
    if (!is.null(peptide_data) && length(peptide_data$genomic_ranges) > 0) {
      ...
      # Only add if we have consistent data
      if (length(tx_peptides) > 0 && 
          length(start(tx_peptides)) == length(tx_peptides) &&
          length(tx_peptides$peptide) == length(tx_peptides)) {
        
        # Add all peptide ranges with their metadata and hover text
        peptide_df <- rbind(peptide_df, data.frame(
          transcript = rep(tx, length(tx_peptides)),
          y_position = rep(i, length(tx_peptides)),
  ...
  # Add hover text information to peptides (only if we have peptides)
  if (nrow(peptide_df) > 0) {
    ...
    # Create empty hover_text column for empty peptide_df
    peptide_df$hover_text <- character(0)
  }
  
  # Create the plot - COMBINED VERSION: Show transcript structure WITH peptides
  p <- ggplot() +
    # Add transcript lines (introns)
    geom_segment(data = transcript_df, 
                aes(x = gene_start, xend = gene_end, 
    ...
    # Add peptides with hover information ON TOP of transcript structure
    {if (nrow(peptide_df) > 0) {
  ...
  p <- p + 
    scale_fill_manual(
      values = c(
        ...
        "AS-specific" = "#E74C3C"   # Bright red for AS-specific peptides
      ),
      name = "",
      labels = c(
        "CDS" = "CDS",
        "Regular" = paste0(protease, " peptides"),
        "AS-specific" = "AS-specific peptides"
      )
    ) +
    theme_minimal() +
    theme(
