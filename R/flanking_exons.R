...
search_all_exons_in_cds <- function(orientation_result, cds_index_path = "real_cds_index.RDS") {
  ...
  # NOTE: CDS index functionality will need to be adapted for main app
  # For now, we'll implement a simplified version that works with main app's data structures
  ...
  # Simplified CDS matching for main app integration
  # This would need to be connected to the main app's GTF/CDS data structures
  
  cat("\nINCLUSION isoform exact matches:\n")
  inclusion_matches <- check_exact_exon_matches_simplified(inclusion_isoform, chromosome, strand, gene_id, event_type, "inclusion")
  
  cat("\nEXCLUSION isoform exact matches:\n")
  exclusion_matches <- check_exact_exon_matches_simplified(exclusion_isoform, chromosome, strand, gene_id, event_type, "exclusion")
...
# Simplified exon match check for main app integration
check_exact_exon_matches_simplified <- function(isoform_exons, chromosome, strand, gene_id, event_type, isoform_type) {
  ...
  for (i in seq_along(isoform_exons)) {
    ...
    # Enhanced CDS search: try real lookup first, fallback to simplified
    if (exists("rmats_cds_index") && !is.null(rmats_cds_index)) {
      # Try real CDS index search (seamless enhancement)
      cds_result <- search_cds_index(rmats_cds_index, chromosome, exon$start, exon$end, strand, gene_id)
    ...
    exon_matches[[i]] <- list(
      ...
      phase = phase,  # Now real phase when available!
...
analyze_cds_coverage <- function(inclusion_matches, exclusion_matches, event_type) {
  ...
  # Count CDS exons
  inclusion_cds_count <- sum(sapply(inclusion_matches, function(x) x$exact_match))
  exclusion_cds_count <- sum(sapply(exclusion_matches, function(x) x$exact_match))
