...
source("utils/coordinate_utils.R")

# Function to validate that file structure matches selected event type
validate_event_type_compatibility <- function(rmats_data, selected_event_type) {
  colnames_data <- colnames(rmats_data)
  
  if (selected_event_type == "SE") {
    required_cols <- c("exonStart_0base", "exonEnd", "upstreamES", "upstreamEE", "downstreamES", "downstreamEE")
    if (all(required_cols %in% colnames_data)) {
      return(TRUE)
    }
  } else if (selected_event_type == "A3SS" || selected_event_type == "A5SS") {
    # A3SS and A5SS have identical column structures - rely on user selection
    required_cols <- c("longExonStart_0base", "longExonEnd", "shortES", "shortEE", "flankingES", "flankingEE")
    if (all(required_cols %in% colnames_data)) {
      return(TRUE)
    }
  } else if (selected_event_type == "MXE") {
    # Handle both original and X-prefixed column names
    required_cols_original <- c("1stExonStart_0base", "1stExonEnd", "2ndExonStart_0base", "2ndExonEnd", "upstreamES", "upstreamEE", "downstreamES", "downstreamEE")
    required_cols_x_prefix <- c("X1stExonStart_0base", "X1stExonEnd", "X2ndExonStart_0base", "X2ndExonEnd", "upstreamES", "upstreamEE", "downstreamES", "downstreamEE")
    if (all(required_cols_original %in% colnames_data) || all(required_cols_x_prefix %in% colnames_data)) {
      return(TRUE)
    }
  } else if (selected_event_type == "RI") {
    required_cols <- c("riExonStart_0base", "riExonEnd", "upstreamES", "upstreamEE", "downstreamES", "downstreamEE")
    if (all(required_cols %in% colnames_data)) {
      return(TRUE)
    }
  }
...
# Legacy function for backward compatibility - now just validates compatibility
detect_event_type <- function(rmats_data, selected_type = NULL) {
  if (!is.null(selected_type)) {
    # If user provided a type, validate compatibility
    if (validate_event_type_compatibility(rmats_data, selected_type)) {
      return(selected_type)
    } else {
      stop(paste("File structure does not match selected event type:", selected_type))
    }
  }
  
  # Fallback: try to detect for backward compatibility
  colnames_data <- colnames(rmats_data)
  ...
  if ("exonStart_0base" %in% colnames_data && "upstreamES" %in% colnames_data) {
    ...
    # Could be A3SS or A5SS - cannot distinguish, return generic message
    warning("File appears to be A3SS/A5SS format - please select event type manually")
    return("A3SS_OR_A5SS")
